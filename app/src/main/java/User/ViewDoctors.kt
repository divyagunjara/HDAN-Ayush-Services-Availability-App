...
class ViewDoctors : AppCompatActivity() {
            ...
            RetrofitClient.instance.admintypestatus("Doctor","Available","admintypestatus")
                .enqueue(object : Callback<Userresponse> {
                    ...
                    override fun onResponse(call: Call<Userresponse>, response: Response<Userresponse>) {
                        ...
                        Toast.makeText(this@ViewDoctors, "${response.body()!!.message}", Toast.LENGTH_SHORT).show()
                        listdoc.adapter=Userdocsadapter(this@ViewDoctors,response.body()!!.user,place)
                        p.dismiss()
                    }
    ...
    private fun readcity(city: String) {
            ...
            p.show()
            CoroutineScope(Dispatchers.IO).launch {
                RetrofitClient.instance.usersstatusview("Doctor","${city}","Available","adminuserstatus")
                    .enqueue(object : Callback<Userresponse> {
                        @SuppressLint("SetTextI18n")
                        override fun onResponse(call: Call<Userresponse>, response: Response<Userresponse>) {
    ...
    class Userdocsadapter(var context: Context, var listdata: ArrayList<User>, var place:String):
        RecyclerView.Adapter<Userdocsadapter.DataViewHolder>(){
        var id=0
        class DataViewHolder(view: View) : RecyclerView.ViewHolder(view) {
            ...
            val btnappointmnet:Button=view.findViewById(R.id.btnappointmnet)


        }

        override fun onCreateViewHolder(parent: ViewGroup, viewType: Int): DataViewHolder {
            val view = LayoutInflater.from(context).inflate(R.layout.carduserdoc, parent, false)
            ...
            holder.apply {
                listdata.get(position).apply {
                    ...
                    holder.btnfeedback.setOnClickListener {
                        val ii=Intent(context,viewfeedbacks::class.java)
                        ii.putExtra("demail",email)
                        context.startActivity(ii)
                    }


                    holder.btnappointmnet.setOnClickListener {
                        val ii=Intent(context,Addappointment::class.java)
                        ii.putExtra("dname",name)
                        ii.putExtra("demail",email)
                        ii.putExtra("dnum",moblie)
                        context.startActivity(ii)
